...
<script type="text/javascript" src="<?=PATHSITE?>js2/scripts.js?v=1.0.6"></script>
   ...
   <script>
    ...
    $.post("<?= PATHSITE ?>" + "produto/adicionaData/", {"mes": mes, "ano": ano, "dia": dia, "id": id, "tipo": "REM"}, function (retorno) {
    seletor.removeClass("ocupado");
    });
    } else {
    $.post("<?= PATHSITE ?>" + "produto/adicionaData/", {"mes": mes, "ano": ano, "dia": dia, "id": id, "tipo": "ADD"}, function (retorno) {
   ...
   function ordena(lista, tabela, nomeModel) {
        $.post('<?= PATHSITE ?>utils/ordena/', {lista: lista, tabela: tabela, nomeModel: nomeModel}, function (retorno) {
...
<script>$('.ui-sortable-handle').draggable();</script>
